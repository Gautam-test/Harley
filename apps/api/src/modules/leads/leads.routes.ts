...
function requireVerifiedToken(purpose: 'ENQUIRY' | 'GENERAL_LEAD' | 'TRADE_IN') {
...
publicLeadsRouter.post(
  '/general',
  requireVerifiedToken('GENERAL_LEAD'),
  validate(generalLeadInput),
  async (req, res, next) => {
    try {
      const out = await createGeneralLead(req.body);
...
dealerLeadsRouter.get('/general', validate(statusQuery, 'query'), async (req, res, next) => {
  try {
    res.json(await listGeneralLeads(req.auth!.sub, (req.query as { status?: LeadStatus }).status));
  } catch (e) {
    next(e);
  }
});
...
dealerLeadsRouter.get(
  '/:kind/:id/detail',
  validate(
    z.object({ kind: z.enum(['general', 'buyer', 'trade-in']), id: z.string().min(1) }),
    'params',
  ),
  async (req, res, next) => {
    try {
...
dealerLeadsRouter.patch(
  '/:kind/:id/status',
  validate(z.object({ kind: z.enum(['general', 'buyer', 'trade-in']), id: z.string().min(1) }), 'params'),
  validate(statusBody),
  async (req, res, next) => {
    try {
      const { kind, id } = req.params as { kind: 'general' | 'buyer' | 'trade-in'; id: string };
      ...
      void audit({
        ...
        entityType: kind === 'buyer' ? 'Enquiry' : kind === 'general' ? 'GeneralLead' : 'TradeInLead',
        entityId: id,
